...
class PlaceInfo(BaseModel):
    ...
    address: Optional[str] = Field(None, description="Place address (street address preferred)")
    ...
    recommendation_score: int = Field(..., ge=1, le=10, description="Recommendation score (1-10)")
...
class PlaceAnalysisRequest(BaseModel):
    ...
    hashtags: List[str] = Field(default_factory=list, description="Hashtags from content")
    images: List[str] = Field(default_factory=list, description="Image URLs for vision analysis")
...
class PlaceAnalysisResponse(BaseModel):
    ...
    place_info: Optional[PlaceInfo] = Field(None, description="Extracted place information")
    confidence: float = Field(0.0, ge=0.0, le=1.0, description="Analysis confidence score")
