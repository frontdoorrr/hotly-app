...
from app.api.deps import get_current_user, get_db, get_cache_service
    ...
    try:
        ...
        if paginated_response:
            return SearchOptimizedResponse(
                ...
                has_more=paginated_response.get('pagination', {}).get('has_next', False),
...
async def record_client_performance_metrics(
    ...
    try:
        ...
        if success:
            return JSONResponse(
                content={"message": "성능 메트릭이 기록되었습니다"},
                status_code=status.HTTP_200_OK
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="메트릭 기록에 실패했습니다"
            )
...
async def clear_search_history(
    ...
    try:
        ...
        if success:
            return JSONResponse(
                content={"message": "검색 히스토리가 삭제되었습니다"}
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="히스토리 삭제에 실패했습니다"
            )
...
async def clear_search_cache(
    ...
    try:
        ...
        return JSONResponse(
            content={"message": f"{cache_type} 캐시가 삭제되었습니다"}
        )
