...
@router.get(
    "/performance-ranking",
    summary="API 성능 순위",
    description="API 엔드포인트의 성능 순위를 조회합니다."
)
...
@router.get(
    "/slow-queries",
    summary="느린 쿼리 조회",
    description="성능 임계값을 초과한 느린 쿼리와 최적화 제안을 조회합니다."
)
...
@router.get(
    "/trend-analysis",
    summary="성능 트렌드 분석",
    description="지정된 기간 동안의 성능 트렌드를 분석합니다."
)
...
@router.delete(
    "/metrics/clear",
    summary="메트릭 데이터 초기화",
    description="수집된 모든 성능 메트릭 데이터를 초기화합니다."
)
...
@router.get(
    "/health",
    summary="성능 모니터링 시스템 상태",
    description="성능 모니터링 시스템의 상태를 확인합니다."
)
async def get_monitoring_health():
    ...
    try:
        ...
        health_status = {
            ...
            "metrics_collector": {
                "api_metrics_count": sum(len(times) for times in collector.api_response_times.values()),
