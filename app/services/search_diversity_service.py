...
class SearchDiversityService:
    ...
    def _analyze_result_clusters(self, search_results: List[Dict[str, Any]]) -> Dict[str, List[int]]:
        """검색 결과를 클러스터별로 분석"""
        clusters = {
    ...
    def _calculate_diversity_scores(
        ...
        for i, result in enumerate(search_results):
            ...
            price_range = result.get('price_range', 0)
            price_cluster = self._get_price_cluster(price_range)
            price_frequency = len(result_clusters['price_ranges'].get(price_cluster, []))
            ...
            # 특성 다양성 (태그/키워드 기반)
            feature_diversity = self._calculate_feature_diversity(result, search_results)
    ...
    def _balance_personalization_and_diversity(
        ...
        for i, result in enumerate(search_results):
            ...
            # 가중 평균으로 최종 점수 계산
            final_score = (
                (1 - diversity_factor) * personalization_score +
    ...
    async def _remove_similar_results(self, search_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """유사한 결과 제거"""
        ...
        try:
            ...
            logger.info(f"Removed {len(search_results) - len(unique_results)} similar results")
    ...
    def _get_category_weights(self, user_preferences: Optional[Dict[str, Any]]) -> Dict[str, float]:
        """카테고리 가중치 계산"""
    ...
    def _calculate_category_quotas(
        ...
        unweighted_categories = [cat for cat in categories if cat not in category_weights]
        ...
        for category in weighted_categories:
            ...
            quota = min(
                available_items,
                max(1, int(total_results * weight))
            )
    ...
    def _calculate_diversity_metrics(self, search_results: List[Dict[str, Any]]) -> Dict[str, float]:
        """다양성 메트릭 계산"""
        if not search_results:
        ...
        # 가격대 분포
        price_ranges = [self._get_price_cluster(r.get('price_range', 0)) for r in search_results]
        price_diversity = self._calculate_shannon_diversity(price_ranges)
        
        # 지역 분포
        regions = [self._get_region_cluster(r.get('region', '')) for r in search_results]
    ...
    def _apply_diversity_adjustments(
        ...
        for dimension in low_diversity_dimensions:
            adjusted_results = self._adjust_dimension_diversity(adjusted_results, dimension)
    ...
    def _adjust_category_diversity(self, search_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """카테고리 다양성 조정"""
        category_groups = defaultdict(list)
        for i, result in enumerate(search_results):
            category = result.get('category', 'unknown')
    ...
    def _adjust_region_diversity(self, search_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """지역 다양성 조정"""
    ...
    def _adjust_price_diversity(self, search_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """가격대 다양성 조정"""
