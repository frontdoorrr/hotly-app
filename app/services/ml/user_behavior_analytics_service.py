...
from app.services.monitoring.logging_service import LogLevel, logging_service
    ...
    def analyze_user_journey(self, user_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "conversion_rate": len(journey.conversion_events)
                / max(len(actions), 1),
            }
...
class BehaviorAnalyzer:
    ...
    def create_user_profile(
        self, user_id: str, demographics: Dict, behavior_data: List[Dict]
    ):
        """사용자 프로필 생성"""
        with self._lock:
    ...
    def _analyze_behavior(self, behavior_data: List[Dict]) -> Dict[str, Any]:
        ...
        search_count = sum(
            b.get("count", 1) for b in behavior_data if b["action"] == "search"
        )
        view_count = sum(
            b.get("count", 1) for b in behavior_data if b["action"] == "view"
        )
        favorite_count = sum(
            b.get("count", 1) for b in behavior_data if b["action"] == "favorite"
        )
    ...
    def analyze_search_patterns(self, user_actions: List[Dict]) -> Dict[str, Any]:
        ...
        most_common_location = (
            max(set(locations), key=locations.count) if locations else None
        )
    ...
    def analyze_engagement_patterns(self, user_actions: List[Dict]) -> Dict[str, Any]:
        ...
        most_common_sequence = (
            max(set(action_sequences), key=action_sequences.count)
            if action_sequences
            else None
        )
...
class AnalyticsDashboard:
    ...
    def collect_behavior_metrics(self, behavior_data: List[Dict]) -> Dict[str, Any]:
        ...
        for action in behavior_data:
            ...
            sessions[session_id]["end_time"] = max(
                sessions[session_id]["end_time"], action["timestamp"]
            )
    ...
    def collect_conversion_metrics(self, conversion_data: List[Dict]) -> Dict[str, Any]:
        ...
        conversions = sum(
            1 for event in conversion_data if event["event_type"] == "conversion"
        )
    ...
    def add_live_action(self, action: Dict[str, Any]):
        ...
        with self._lock:
            ...
            if len(self.real_time_data["live_actions"]) > 100:
                self.real_time_data["live_actions"] = self.real_time_data[
                    "live_actions"
                ][-100:]
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        """대시보드 데이터 조회"""
        with self._lock:
            return {**self.dashboard_data, "real_time_data": self.real_time_data.copy()}
...
class UserBehaviorAnalyticsService:
    ...
    def track_action(
        ...
        self.analytics_dashboard.add_live_action(
            {"action_type": action_type, "user_id": user_id, **action_data}
        )
    ...
    def update_dashboard_metrics(
        ...
        if user_data:
            dashboard_data[
                "user_metrics"
            ] = self.analytics_dashboard.collect_user_metrics(user_data)

        if behavior_data:
            dashboard_data[
                "behavior_metrics"
            ] = self.analytics_dashboard.collect_behavior_metrics(behavior_data)

        if conversion_data:
            dashboard_data[
                "conversion_metrics"
            ] = self.analytics_dashboard.collect_conversion_metrics(conversion_data)
...
# 편의 함수들
def track_user_action(
    user_id: str, action_type: str, action_data: Dict[str, Any], session_id: str = None
):
    """사용자 액션 추적 편의 함수"""
    return user_analytics_service.track_action(
...
def track_favorite_action(
    ...
    """즐겨찾기 액션 추적 편의 함수"""
    action_type = (
        ActionType.ADD_FAVORITE if action == "add" else ActionType.REMOVE_FAVORITE
    )
    return track_user_action(
        user_id=user_id,
        action_type=action_type.value,
        action_data={"place_id": place_id, "place_name": place_name},
        session_id=session_id,
    )
