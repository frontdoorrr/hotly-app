...
from app.models.notification import UserNotificationSettings, NotificationType
...
class PersonalizedNotificationService:
        ...
        try:
            ...
            if optimize_timing:
                try:
                    ...
                    logger.info(f"Timing optimized for user {user_id}: {optimization_info}")
                    
                except Exception as e:
                    logger.warning(f"Timing optimization failed for user {user_id}: {e}")
            ...
            current_time = datetime.utcnow()
            send_immediately = (
                not optimize_timing or 
                optimized_time is None or
                optimized_time <= current_time + timedelta(minutes=5)  # Send if within 5 minutes
            )
            ...
            if send_immediately:
                ...
                logger.info(f"Scheduling notification for user {user_id} at {optimized_time}")
            ...
            logger.error(f"Failed to send personalized notification to user {user_id}: {e}")
            return {
                "success": False,
                "error": str(e),
                "user_id": user_id
    ...
    async def send_bulk_personalized_notifications(
        ...
        allowed_requests = [
            req for req in notification_requests 
        ...
        if allowed_requests:
            ...
            for request in allowed_requests:
                timing_requests.append(PersonalizedTimingRequest(
                    user_id=request["user_id"],
                    notification_type=request["notification_request"].notification_type,
                    default_time=datetime.utcnow()
                ))
            
            try:
                predictions = await self.personalization_engine.optimize_batch_timing(timing_requests)
            except Exception as e:
            ...
            for i, request in enumerate(allowed_requests):
                ...
                try:
                    ...
                    if optimized_time <= current_time + timedelta(minutes=5):
                        ...
                        response = self.fcm_service.send_push_notification(notification_req)
        ...
        logger.info(f"Bulk personalized notifications complete: {results['optimization_stats']}")
        return results
    ...
    async def get_user_notification_insights(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            behavior_metrics = await self.personalization_engine.analyze_user_behavior_patterns(user_id)
            ...
            insights = {
                ...
                "recommendations": self._generate_user_recommendations(settings, behavior_metrics)
            }
