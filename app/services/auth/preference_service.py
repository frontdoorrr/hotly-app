...
class PreferenceSetupService:
    ...
    def setup_initial_categories(
        ...
        logger.info(
            f"Initial categories setup for user {user_id}: {selected_categories}"
        )
    ...
    def calculate_preference_quality_score(
        ...
        if location_prefs.get("current_location") and location_prefs.get(
            "preferred_areas"
        ):
        ...
        if companion_prefs.get("companion_type") and companion_prefs.get(
        ...
        completed_sections = sum(
            1 for section in required_sections if section in preference_data
        )
...
class PreferenceSurveyService:
    ...
    def generate_adaptive_survey(
        ...
        survey_questions = self._select_relevant_questions(
            initial_preferences, estimated_questions
        )
    ...
    def process_survey_response(
        ...
        adaptation_signals = self._analyze_response(
    ...
    def _select_relevant_questions(
        ...
        for question_id, question_data in list(self.survey_questions.items())[
            :target_count
        ]:
    ...
    def _get_next_question(
        self, survey_id: str, signals: Dict[str, Any]
    ) -> Optional[str]:
        ...
        if normalization_method == "softmax":
            ...
            for category, weights in category_weights.items():
                normalized_weights[category] = (
                    sum(weights.values()) / total if total > 0 else 0
                )
        else:  # linear
            normalized_weights = {}
            for category, weights in category_weights.items():
                normalized_weights[category] = (
                    sum(weights.values()) / len(weights) if weights else 0
                )
...
class PreferenceSurveyService:
    ...
    def generate_adaptive_survey(
        self, user_id: str, previous_answers: List[Dict], adaptive_questions: List[Dict]
    ) -> Dict[str, Any]:
...
class PreferencePersonalizationService:
    ...
    def create_personalized_onboarding(
        ...
        customized_questions = [
            ...
            {
                "id": "location_based",
                "question": "현재 위치 근처의 장소를 주로 찾으시나요?",
            },
        ]
    ...
    def learn_from_behavior(
        ...
        return {
            ...
            "behavior_insights": {
                "most_engaged_category": max(
                    time_spent_per_category, key=time_spent_per_category.get
                ),
    ...
    def assess_preference_quality(
        ...
        response_score = min(
            detailed_responses / 10, 1.0
        )  # Assuming 10 responses optimal
        ...
        return {
            ...
            "recommendation_readiness": {
                ...
                "improvements_needed": (
                    [] if quality_score > 0.6 else ["더 많은 카테고리 선택", "상세 응답 추가"]
                ),
            },
