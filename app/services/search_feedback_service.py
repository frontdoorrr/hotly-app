...
class SearchFeedbackService:
    ...
    def __init__(self, db: AsyncSession, cache_service: CacheService, ml_engine: MLEngine):
        """서비스 초기화"""
        self.db = db
    ...
    async def process_feedback_batch(self, user_id: Optional[UUID] = None) -> Dict[str, Any]:
        """
        배치 피드백 처리 및 모델 업데이트
        ...
        try:
            ...
            if len(feedback_batch) < self.learning_threshold:
                logger.info(f"Insufficient feedback for learning: {len(feedback_batch)}")
                return {"processed": 0, "learned": False}
            
            # 피드백 데이터를 학습 데이터로 변환
            training_data = await self._convert_feedback_to_training_data(feedback_batch)
            ...
            profile_updates = 0
            if user_id:
                updated = await self._update_user_profile_from_feedback(user_id, feedback_batch)
                if updated:
                    profile_updates = 1
            else:
                # 모든 사용자 프로필 업데이트
                ...
                for uid_str in user_ids:
                    uid = UUID(uid_str)
                    user_feedback = [fb for fb in feedback_batch if fb['user_id'] == uid_str]
                    updated = await self._update_user_profile_from_feedback(uid, user_feedback)
                    if updated:
                        profile_updates += 1
    ...
    async def analyze_feedback_patterns(
        ...
        try:
            ...
            feedback_data = await self._get_user_feedback_history(user_id, start_time, end_time)
            ...
            patterns = {
                ...
                "feedback_distribution": self._analyze_feedback_distribution(feedback_data),
                "interaction_quality": self._calculate_interaction_quality(feedback_data),
                "temporal_patterns": self._analyze_temporal_patterns(feedback_data),
                "category_preferences": await self._analyze_category_preferences(feedback_data),
                "search_success_rate": self._calculate_search_success_rate(feedback_data),
    ...
    async def _collect_feedback_batch(self, user_id: Optional[UUID] = None) -> List[Dict[str, Any]]:
        """처리할 피드백 배치 수집"""
        queue_key = "search_feedback_queue"
        queue_data = await self.cache.get(queue_key) or []
    ...
    async def _extract_features_from_feedback(self, feedback: Dict[str, Any]) -> Dict[str, float]:
        """피드백에서 ML 특성 추출"""
        features = {
            'rating': 0.0,
            'distance': 0.0,
        ...
        if 'location_type' in context:
            features['location_context'] = self._encode_location_context(context['location_type'])
    ...
    async def _update_user_profile_from_feedback(
        ...
        try:
            ...
            positive_feedback = [
                ...
                if FeedbackType(fb['feedback_type']) in [
                    FeedbackType.CLICK, FeedbackType.BOOKMARK, 
                    FeedbackType.VISIT, FeedbackType.SHARE
                ]
            ]
    ...
    def _analyze_feedback_distribution(self, feedback_data: List[Dict[str, Any]]) -> Dict[str, int]:
        """피드백 분포 분석"""
        distribution = {}
    ...
    def _calculate_interaction_quality(self, feedback_data: List[Dict[str, Any]]) -> float:
        """상호작용 품질 계산"""
        if not feedback_data:
            return 0.0
    ...
    def _analyze_temporal_patterns(self, feedback_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """시간적 패턴 분석"""
        hourly_distribution = {}
        daily_distribution = {}
    ...
    async def _analyze_category_preferences(self, feedback_data: List[Dict[str, Any]]) -> Dict[str, float]:
        """카테고리 선호도 분석"""
        category_scores = {}
    ...
    def _calculate_search_success_rate(self, feedback_data: List[Dict[str, Any]]) -> float:
        """검색 성공률 계산"""
        if not feedback_data:
            return 0.0
        ...
        positive_interactions = sum(
            ...
            if FeedbackType(feedback.get('feedback_type', 'view')) in [
                FeedbackType.CLICK, FeedbackType.BOOKMARK, 
                FeedbackType.VISIT, FeedbackType.SHARE
            ]
        )
