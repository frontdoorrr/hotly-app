...
from app.exceptions.ai import AIAnalysisError, RateLimitError, InvalidResponseError, AIServiceUnavailableError
...
class GeminiAnalyzer:
    ...
    async def analyze_place_content(self, request: PlaceAnalysisRequest) -> PlaceInfo:
        ...
        try:
            ...
            response_text = await self._call_gemini_api_with_retry(formatted_prompt, request.images)
    ...
    def _format_prompt(self, request: PlaceAnalysisRequest) -> str:
        ...
        return PLACE_EXTRACTION_PROMPT_V1.format(
            ...
            json_schema=json.dumps(PLACE_EXTRACTION_JSON_SCHEMA, indent=2, ensure_ascii=False)
        )
    ...
    def _parse_and_validate_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise InvalidResponseError(f"Response does not match expected schema: {str(e)}")
        except Exception as e:
