...
class FilterService:
    ...
    def _apply_sorting(self, query: Query, sort_criteria: SortCriteria) -> Query:
        ...
        for sort_field in sort_criteria.fields:
            ...
            if sort_field.field == SortField.NAME:
                order_by = Place.name.asc() if direction == SortDirection.ASC else Place.name.desc()
            
            elif sort_field.field == SortField.CREATED_AT:
                order_by = Place.created_at.asc() if direction == SortDirection.ASC else Place.created_at.desc()
            
            elif sort_field.field == SortField.UPDATED_AT:
                order_by = Place.updated_at.asc() if direction == SortDirection.ASC else Place.updated_at.desc()
            
            elif sort_field.field == SortField.RATING:
                order_by = Place.rating.asc() if direction == SortDirection.ASC else Place.rating.desc()
            
            elif sort_field.field == SortField.PRICE_RANGE:
                order_by = Place.price_range.asc() if direction == SortDirection.ASC else Place.price_range.desc()
            
            elif sort_field.field == SortField.VISIT_COUNT:
                order_by = Place.visit_count.asc() if direction == SortDirection.ASC else Place.visit_count.desc()
                ...
                if hasattr(sort_criteria, 'user_location') and sort_criteria.user_location:
                    ...
                    order_by = distance_formula.asc() if direction == SortDirection.ASC else distance_formula.desc()
                else:
    ...
    async def _calculate_filter_stats(
        ...
        # 전체 장소 개수
        total_places = await self.db.query(Place).filter(
        ...
        if criteria.categories:
            for category in criteria.categories:
                count = await self.db.query(Place).filter(
                    Place.user_id == user_id,
        ...
        filter_efficiency = (total_places - filtered_count) / total_places if total_places > 0 else 0
    ...
    async def get_available_filters(self, user_id: UUID) -> Dict[str, List[str]]:
        ...
        user_places = await self.db.query(Place).filter(
            Place.user_id == user_id
        ).all()
        
        available_filters = {
            "categories": list(set(place.category for place in user_places if place.category)),
            "regions": list(set(place.region for place in user_places if place.region)),
            "price_ranges": list(set(str(place.price_range) for place in user_places if place.price_range)),
            "tags": list(set(
                tag.name for place in user_places for tag in place.tags
            )),
    ...
    async def save_filter_preset(
        ...
        # 기존 프리셋 확인
        existing_preset = await self.db.query(FilterPreset).filter(
    ...
    async def get_filter_presets(self, user_id: UUID) -> List[FilterPreset]:
        ...
        presets = await self.db.query(FilterPreset).filter(
            FilterPreset.user_id == user_id
        ).order_by(FilterPreset.updated_at.desc()).all()
    ...
    async def get_recommended_filters(
        ...
        # 즐겨찾기 + 미방문 조합 추천
        favorite_count = await self.db.query(Place).filter(
            Place.user_id == user_id,
        ...
        # 최근 저장된 장소 추천
        recent_places_count = await self.db.query(Place).filter(
            Place.user_id == user_id,
        ...
        if recent_places_count > 0:
            recommendation = FilterRecommendation(
                ...
                criteria=FilterCriteria(
                    created_after=datetime.utcnow().replace(day=1)
                ),
    ...
    async def _track_filter_usage(
        ...
        usage_data = {
            ...
            "has_rating_filter": criteria.min_rating is not None or criteria.max_rating is not None,
            "sort_fields": [f.field.value for f in criteria.sort_criteria.fields] if criteria.sort_criteria else [],
        }
