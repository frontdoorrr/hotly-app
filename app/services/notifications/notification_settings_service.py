...
class NotificationSettingsService:
    ...
    async def create_user_settings(
        self, user_id: str, settings_data: UserNotificationSettingsCreate
    ...
    async def get_user_settings(self, user_id: str) -> UserNotificationSettings:
        ...
        if not settings:
            raise NotificationSettingsNotFoundError(
                f"Notification settings not found for user {user_id}"
            )

        return settings

    async def update_user_settings(
        self, user_id: str, update_data: UserNotificationSettingsUpdate
    ) -> UserNotificationSettings:
        """
        ...
        if update_data.timing is not None:
            ...
            settings.departure_minutes_before = (
                update_data.timing.departure_minutes_before
            )
        ...
        if update_data.personalization is not None:
            ...
            settings.frequency_limit_per_day = (
                update_data.personalization.frequency_limit_per_day
            )
            settings.frequency_limit_per_week = (
                update_data.personalization.frequency_limit_per_week
            )
    ...
    def is_notification_allowed_for_user(
        self, user_id: str, notification_type: str, current_time: datetime = None
    ) -> bool:
        """
        ...
        try:
            ...
            logger.error(
                f"Error checking notification permission for user {user_id}: {e}"
            )
            # In case of error, be permissive and allow the notification
            return True

    async def bulk_check_notification_permissions(
        self, user_notification_pairs: list, current_time: datetime = None
    ) -> dict:
        """
        ...
        user_settings = (
            self.db.query(UserNotificationSettings)
            .filter(UserNotificationSettings.user_id.in_(user_ids))
            .all()
        )

        # Create lookup dictionary
        settings_lookup = {
            str(settings.user_id): settings for settings in user_settings
        }
        ...
        for user_id, notification_type in user_notification_pairs:
            ...
            if settings:
                allowed = settings.is_notification_allowed(
                    notification_type, current_time
                )
            else:
