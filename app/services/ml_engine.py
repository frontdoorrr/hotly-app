...
class MLEngine:
    ...
    def __init__(self, cache_service: CacheService):
        ...
        self.personalization_model = SGDRegressor(
            alpha=0.01,
            random_state=42
        )
    ...
    async def get_model_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.is_trained and hasattr(self.relevance_model, 'feature_importances_'):
    ...
    async def _predict_personalization(
        ...
        try:
            ...
            if context:
                ...
                user_weights = self._apply_context_to_weights(user_weights, context_adjustment)
            
            # 개인화 점수 계산
            personalization_scores = self.personalization_model.predict(features)
            
            # 사용자 가중치 적용
            adjusted_scores = personalization_scores * user_weights.get('global_factor', 1.0)
    ...
    async def _get_default_scores(self, feature_vectors: List[Dict[str, Any]]) -> List[float]:
    ...
    def _convert_features_to_array(self, feature_vectors: List[Dict[str, Any]]) -> np.ndarray:
        """특성 벡터를 numpy 배열로 변환"""
        feature_names = self._get_feature_names()
        features_array = []
    ...
    def _prepare_training_data(self, feedback_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """피드백 데이터를 훈련 데이터로 변환"""
        training_data = []
    ...
    def _prepare_training_features_and_labels(
        ...
        for item in data:
            try:
                feature_vector = self._convert_features_to_array([item.get('features', {})])
    ...
    async def _perform_online_learning(self, training_data: List[Dict[str, Any]]) -> bool:
        """온라인 학습 수행"""
        try:
    ...
    async def _get_user_personalization_weights(self, user_id: UUID) -> Dict[str, float]:
        """사용자별 개인화 가중치 조회"""
    ...
    async def _load_model(self):
        ...
        try:
            ...
            self.personalization_model = pickle.loads(model_data['personalization_model'])
