        ...
        def decorator(func: Callable):
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    # 엔드포인트 통계 업데이트
                    self.track_endpoint_performance(endpoint, method, duration_ms, status_code)
...
class PerformanceDashboard:
    ...
    def _check_alerts(self, metrics: Dict[str, Any], timestamp: str):
        ...
        # 캐시 적중률 체크
        if metrics.get("cache_hit_rate", 1.0) < self.alert_thresholds["cache_hit_rate_min"]:
            self.alerts.append({
    ...
    def analyze_performance_trend(self, hours: int = 1) -> Dict[str, Any]:
        ...
        if len(recent_metrics) >= 2:
            ...
            response_time_trend = (
                "improving" if last.response_time < first.response_time
                else ("degrading" if last.response_time > first.response_time else "stable")
            )
            cache_trend = (
                "improving" if last.cache_hit_rate > first.cache_hit_rate
                else ("degrading" if last.cache_hit_rate < first.cache_hit_rate else "stable")
            )
            error_trend = (
                "improving" if last.error_rate < first.error_rate
