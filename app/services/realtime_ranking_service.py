...
class RealtimeRankingService:
    ...
    async def trigger_ranking_update(
        ...
        try:
            # 업데이트 필요 여부 확인
            if not force and not await self._should_update_ranking(user_id, trigger_type, context):
                return False
    ...
    async def update_popularity_scores(self, place_ids: List[UUID]) -> bool:
        ...
        try:
            ...
            if popularity_updates:
                ...
                logger.info(f"Updated popularity scores for {len(popularity_updates)} places")
    ...
    async def _notify_ranking_update(self, user_id: UUID, trigger_type: str, updated_count: int):
        """랭킹 업데이트 알림"""
    ...
    async def _register_search_session(self, user_id: UUID, search_data: Dict[str, Any]):
        """검색 세션 등록"""
        try:
            cache_key = f"active_searches:{user_id}"
            active_searches = await self.cache.get(cache_key) or []
    ...
    async def _check_location_change_threshold(self, user_id: UUID, context: Optional[Dict[str, Any]]) -> bool:
        """위치 변화 임계값 확인"""
        return True
    
    async def _check_time_change_threshold(self, user_id: UUID, context: Optional[Dict[str, Any]]) -> bool:
        """시간 변화 임계값 확인"""  
        return True
    
    async def _check_popularity_change_threshold(self, context: Optional[Dict[str, Any]]) -> bool:
        """인기도 변화 임계값 확인"""
        return True
