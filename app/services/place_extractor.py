...
from app.schemas.ai import GeminiResponse, PlaceAnalysisResult, AnalysisConfidence
...
class PlaceExtractor:
    ...
    def __init__(self):
        ...
        self.valid_categories = {
            ...
            "shopping", "accommodation", "entertainment", "other"
        }
    ...
    async def extract_and_structure_places(
        ...
        try:
            ...
            for place_data in gemini_response.places:
                try:
                    extracted_place = await self._process_single_place(place_data, content)
                    if extracted_place:
                    ...
                    result.validation_errors.append(f"Unexpected error processing place: {str(e)}")
            ...
            result.data_quality_score = self._calculate_data_quality_score(deduplicated_places)
    ...
    async def _process_single_place(
        self, 
        place_data: PlaceAnalysisResult,
        content: ExtractedContent
        ...
        try:
            ...
            extracted_place = ExtractedPlace(
                ...
                description=place_data.description.strip() if place_data.description else None,
            ...
            if structured_address.completeness_score < 0.3:
                extracted_place.validation_warnings.append("Incomplete address information")
            ...
            raise PlaceValidationError(f"Failed to process place '{place_data.name}': {str(e)}")
    ...
    def _convert_confidence(self, confidence: AnalysisConfidence) -> ExtractionConfidence:
    ...
    def _structure_address(self, address: str) -> StructuredAddress:
        ...
        street_match = re.search(r'(\d+[^,]*(?:로|길|street|st|avenue|ave))', full_address, re.IGNORECASE)
        if street_match:
    ...
    def _calculate_place_quality_score(
        ...
        if name and len(name.strip()) > 2:
            name_score = min(1.0, len(name.strip()) / 20)  # Longer names generally better
    ...
    def _are_places_similar(self, place1: ExtractedPlace, place2: ExtractedPlace) -> bool:
        """Check if two places are similar (potential duplicates)."""
        # Name similarity
        name_similarity = SequenceMatcher(None, place1.name.lower(), place2.name.lower()).ratio()
        ...
        if name_similarity > 0.6 and address_similarity > 0.7:  # Moderately similar names + similar addresses
            return True
    ...
    def _calculate_overall_confidence(
        ...
        gemini_confidence: AnalysisConfidence
    ) -> float:
        ...
        avg_place_confidence = sum(
            1.0 if p.confidence == ExtractionConfidence.HIGH else
            0.7 if p.confidence == ExtractionConfidence.MEDIUM else 0.4
            for p in places
        ) / len(places)
