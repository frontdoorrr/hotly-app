...
class CDNManager:
    ...
    async def upload_to_cdn(
        self, local_path: str, cdn_path: str, content_type: str = None
    ) -> CDNUploadResult:
        """CDN에 파일 업로드"""
        try:
            # 콘텐츠 타입 결정
    ...
    def check_invalidation_status(self, invalidation_id: str) -> Dict[str, Any]:
        ...
        request = next(
            (
                r
                for r in self.invalidation_requests
...
class StaticResourceOptimizer:
    ...
    def optimize_image(
        self, original_path: str, quality: int = 85, formats: List[str] = None
    ) -> OptimizedResource:
        """이미지 최적화"""
        if formats is None:
            formats = ["webp", "jpeg"]
    ...
    def bundle_javascript(
        self, entry_files: List[str], output_path: str, minify: bool = True
    ) -> Dict[str, Any]:
        """JavaScript 번들링"""
        # 파일 내용 시뮬레이션
        file_contents = {
...
class StaticResourceCacheManager:
    ...
    def generate_cache_headers(
        self, file_path: str, cache_duration: int = None
    ) -> Dict[str, str]:
        """캐시 헤더 생성"""
    ...
    def generate_preload_hints(
        self, critical_resources: List[Dict[str, str]]
    ) -> List[str]:
        """리소스 프리로딩 힌트 생성"""
        preload_tags = []
