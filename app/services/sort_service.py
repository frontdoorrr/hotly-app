...
from app.schemas.sort import (
    PersonalizedSortCriteria,
    SortPreset,
    SortRecommendation,
    SortResult,
)
...
class SortService:
    ...
    async def _apply_single_sort_field(
        ...
        if sort_field.field == SortField.NAME:
            ...
            # 생성 날짜 정렬
            order_expr = Place.created_at.asc() if asc_order else Place.created_at.desc()

        elif sort_field.field == SortField.UPDATED_AT:
            # 수정 날짜 정렬
            order_expr = Place.updated_at.asc() if asc_order else Place.updated_at.desc()
            ...
            # 가격대 정렬
            order_expr = Place.price_range.asc() if asc_order else Place.price_range.desc()

        elif sort_field.field == SortField.VISIT_COUNT:
            # 방문 횟수 정렬
            order_expr = Place.visit_count.asc() if asc_order else Place.visit_count.desc()
    ...
    async def _analyze_user_behavior(self, user_id: UUID) -> Dict[str, Any]:
        ...
        for place in user_places:
            if place.price_range:
                price_counts[place.price_range] = price_counts.get(place.price_range, 0) + 1
        
        price_preferences = {
            price: count / total_places
        ...
        return {
            ...
            "average_rating": sum(p.rating for p in user_places if p.rating) / len([p for p in user_places if p.rating]) if any(p.rating for p in user_places) else 3.0,
        }
    ...
    def _apply_diversity_factor(self, base_score, diversity_factor: float):
        ...
        category_penalty = case(
            [(func.row_number().over(partition_by=Place.category) > 2, -diversity_factor * 0.1)],
            else_=0
        )
        
        # 가격대별 다양성 보장
        price_penalty = case(
            [(func.row_number().over(partition_by=Place.price_range) > 3, -diversity_factor * 0.05)],
            else_=0
        )
    ...
    async def get_recommended_sorts(
        ...
        # 탐험 추천
        unvisited_count = await self.db.query(Place).filter(
            Place.user_id == user_id,
