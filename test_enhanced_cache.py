...
from app.services.cache_manager import CacheManager, CacheKey, DistributedLock, LRUCache
from app.schemas.cache import CacheEntry, CacheStats
...
async def test_enhanced_cache_manager():
    ...
    try:
        ...
        miss_urls = [f"https://missing{i}.com" for i in range(2)]  # These should be misses
    ...
    try:
        ...
        print(f"   50 set operations: {set_time:.3f}s ({set_time/50*1000:.1f}ms per op)")
        print(f"   50 get operations: {get_time:.3f}s ({get_time/50*1000:.1f}ms per op)")
