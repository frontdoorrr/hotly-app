...
from app.schemas.content import ExtractedContent, ContentMetadata, PlatformType
...
def test_extract_content_endpoint_success(client: TestClient) -> None:
    ...
    with patch.object(ContentExtractor, 'extract_content', new_callable=AsyncMock) as mock_extract:
...
def test_extract_content_unsupported_platform(client: TestClient) -> None:
    ...
    with patch.object(ContentExtractor, 'extract_content', new_callable=AsyncMock) as mock_extract:
        mock_extract.side_effect = UnsupportedPlatformError("Unsupported platform: unknown.com")
