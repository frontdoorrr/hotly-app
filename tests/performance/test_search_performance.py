...
class TestSearchPerformance:
    ...
    async def test_large_dataset_search_performance(self) -> None:
        ...
        for query in search_queries:
            ...
            async def mock_large_search(*args, **kwargs):
                ...
                for place in self.large_dataset:
                    if (query.lower() in place["name"].lower() or 
                        query.lower() in place["address"].lower() or
                        query.lower() in place["category"].lower()):
        ...
        p95_response_time = statistics.quantiles([r["response_time"] for r in performance_results], n=20)[18]  # 95%ile
    ...
    async def test_concurrent_search_performance(self) -> None:
        ...
        search_queries = [
            f"테스트 검색 {i}" for i in range(concurrent_users)
        ]
    ...
    async def test_filter_performance_with_large_dataset(self) -> None:
        ...
        # Then: 전체 필터링 성능 검증
        avg_filter_time = statistics.mean([r["response_time"] for r in filter_performance_results])
    ...
    async def test_cache_performance_improvement(self) -> None:
        ...
        # Then: 캐시 전체 성능 개선 검증
        avg_improvement = statistics.mean([r["improvement_ratio"] for r in cache_performance_results])
