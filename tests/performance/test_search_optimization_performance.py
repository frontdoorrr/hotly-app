...
class TestSearchOptimizationPerformance:
    ...
    async def test_single_search_performance(self, mock_services):
        ...
        result = await optimization_service.process_realtime_search(
            query="성능테스트", user_id=user_id, search_type="instant"
        )
        ...
        ), f"Single search too slow: {response_time_ms:.2f}ms"
    ...
    async def test_concurrent_search_performance(self, mock_services):
        ...
        assert (
            total_time_ms < 5000
        ), f"Total concurrent execution too slow: {total_time_ms:.2f}ms"
    ...
    async def test_autocomplete_performance(self, mock_services):
        ...
        for partial_query in partial_queries:
            ...
            assert (
                response_time_ms < 50
            ), f"Autocomplete too slow for '{partial_query}': {response_time_ms:.2f}ms"
    ...
    async def test_pagination_performance(self, mock_services):
        ...
        # 대량 결과 생성
        large_results = [
            {"id": str(i), "name": f"대량결과 {i}", "score": 1000 - i} for i in range(1000)
        ]
        ...
        for page_size in page_sizes:
            ...
            paginated_response = await optimization_service.optimize_pagination(
                results=large_results, request=pagination_request
            )
            ...
            ), f"Pagination too slow for page_size {page_size}: {response_time_ms:.2f}ms"
    ...
    async def test_error_handling_performance(self, mock_services):
        ...
        queries = ["정상검색1", "error검색", "정상검색2", "또다른error", "정상검색3", "정상검색4"]
        ...
        for query in queries:
            ...
            try:
                result = await optimization_service.process_realtime_search(
                    query=query, user_id=uuid4(), search_type="instant"
                )
                success = True
            except Exception:
                result = None
            ...
            ), f"Error handling too slow: {response_time_ms:.2f}ms"
        ...
        print(
            f"  Average success time: {statistics.mean([r['response_time_ms'] for r in successful_requests]):.2f}ms"
        )
        print(
            f"  Average failure time: {statistics.mean([r['response_time_ms'] for r in failed_requests]):.2f}ms"
        )
    ...
    async def test_throughput_benchmark(self):
        ...
        async def benchmark_worker():
            ...
            while time.time() - start_time < duration_seconds:
                await optimization_service.process_realtime_search(
                    query=f"벤치마크{request_count}", user_id=uuid4(), search_type="instant"
                )
                request_count += 1
