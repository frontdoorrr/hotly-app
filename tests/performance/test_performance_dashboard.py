...
class TestPerformanceMetricsCollector:
    ...
    def test_cache_performance_tracking(self):
        ...
        hit_rate = cache_metrics["hit_count"] / (cache_metrics["hit_count"] + cache_metrics["miss_count"])
    ...
    def test_database_query_performance(self):
        ...
        def monitor_query_performance(query: str, execution_time: float, rows_affected: int):
            """쿼리 성능 모니터링"""
        ...
        monitor_query_performance("SELECT * FROM places WHERE category LIKE '%restaurant%'", 150.0, 500)
        monitor_query_performance("INSERT INTO user_activities (...)", 5.0, 1)
        monitor_query_performance("UPDATE places SET updated_at = NOW() WHERE ...", 45.0, 20)
        ...
        avg_time = sum(q["execution_time_ms"] for q in query_metrics) / len(query_metrics)
...
class TestPerformanceDashboard:
    ...
    def test_performance_trend_analysis(self):
        ...
        def add_metrics_snapshot(response_time: float, cache_hit_rate: float, error_rate: float):
            """메트릭 스냅샷 추가"""
        ...
        def analyze_performance_trend(hours: int = 1) -> Dict[str, Any]:
            ...
            if len(recent_metrics) >= 2:
                ...
                response_time_trend = "improving" if last["response_time"] < first["response_time"] else ("degrading" if last["response_time"] > first["response_time"] else "stable")
                cache_trend = "improving" if last["cache_hit_rate"] > first["cache_hit_rate"] else ("degrading" if last["cache_hit_rate"] < first["cache_hit_rate"] else "stable")
                error_trend = "improving" if last["error_rate"] < first["error_rate"] else ("degrading" if last["error_rate"] > first["error_rate"] else "stable")
            else:
        ...
        assert math.isclose(analysis["averages"]["error_rate"], (0.05 + 0.04 + 0.03 + 0.02) / 4)
...
class TestPerformanceOptimization:
    ...
    def test_slow_query_detection(self):
        ...
        def detect_slow_query(query: str, execution_time_ms: float, params: Dict = None):
            """느린 쿼리 감지"""
            if execution_time_ms > query_threshold_ms:
                slow_queries.append({
                    ...
                    "optimization_suggestions": generate_optimization_suggestions(query, execution_time_ms)
                })
        ...
        # When: 다양한 쿼리 시뮬레이션
        detect_slow_query("SELECT id, name FROM places WHERE category = ?", 50, {"category": "restaurant"})
        detect_slow_query("SELECT * FROM places WHERE name LIKE '%pizza%'", 250, {"pattern": "pizza"})
        detect_slow_query("SELECT p.*, r.* FROM places p JOIN reviews r ON p.id = r.place_id", 350)
        detect_slow_query("SELECT COUNT(*) FROM user_activities WHERE created_at > ?", 80)
        ...
        assert any("Optimize JOIN conditions" in suggestion for suggestion in join_query["optimization_suggestions"])
    ...
    def test_api_endpoint_performance_ranking(self):
        ...
        def track_endpoint_performance(endpoint: str, method: str, response_time_ms: float, status_code: int):
            """엔드포인트 성능 추적"""
    ...
    def test_automatic_scaling_recommendations(self):
        ...
        def collect_resource_metrics(cpu: float, memory: float, request_rate: int, response_time: float):
            """리소스 메트릭 수집"""
        ...
        def generate_scaling_recommendations() -> List[Dict[str, Any]]:
            ...
            avg_request_rate = sum(resource_metrics["request_rate"]) / len(resource_metrics["request_rate"])
            avg_response_time = sum(resource_metrics["response_time"]) / len(resource_metrics["response_time"])
        ...
        # 시나리오 2: 높은 CPU 사용률
        resource_metrics = {"cpu_usage": [], "memory_usage": [], "request_rate": [], "response_time": []}
        for _ in range(5):
        ...
        memory_rec = next((r for r in recommendations if r["resource"] == "memory"), None)
        response_rec = next((r for r in recommendations if r["resource"] == "response_time"), None)
