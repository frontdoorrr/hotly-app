...
class TestSearchPersonalizationQuality:
        ...
        self.test_users = [
            {
                ...
                "behavior_history": [
                    {"action": "search", "query": "홍대 카페", "clicked": True, "rating": 4.5},
                    {"action": "visit", "place_id": "cafe_123", "duration_minutes": 120},
                    {"action": "search", "query": "조용한 공간", "clicked": True, "rating": 4.0},
                    {"action": "favorite", "place_id": "cafe_456", "timestamp": "2025-01-10"},
                ],
            },
            {
                "id": uuid4(),
                ...
                "behavior_history": [
                    {"action": "search", "query": "가족 맛집", "clicked": True, "rating": 5.0},
                    {"action": "visit", "place_id": "restaurant_789", "duration_minutes": 90},
                    {"action": "search", "query": "아이와 함께", "clicked": True, "rating": 4.5},
                    {"action": "save", "place_id": "family_place_101", "timestamp": "2025-01-09"},
                ],
            },
        ]
    ...
    async def test_user_behavior_learning_accuracy(self) -> None:
        ...
        # Mock 개인화 학습 서비스
        with patch("app.services.personalization_service.learn_user_preferences") as mock_learning:
            # 행동 기반 선호도 학습 시뮬레이션
            learned_preferences = {
    ...
    async def test_personalized_search_ranking_quality(self) -> None:
        ...
        # Mock 개인화 랭킹 서비스
        with patch("app.services.ranking_service.calculate_personalized_score") as mock_ranking:
            # 카페 선호 사용자용 개인화 점수
            ...
            for place in self.diverse_places:
                ...
                # 가족 사용자 개인화 점수
                family_score = personalized_score_family_user(place, family_user["profile"])
                family_user_results.append({**place, "personalization_score": family_score})
            
            # 개인화 점수순 정렬
            cafe_user_results.sort(key=lambda x: x["personalization_score"], reverse=True)
            family_user_results.sort(key=lambda x: x["personalization_score"], reverse=True)
        ...
        assert (
            cafe_top_result["category"] == "cafe" or 
            cafe_top_result["region"] in ["홍대", "강남"]
        )
        ...
        assert (
            family_top_result["category"] == "restaurant" or
            any(tag in ["가족", "넓은", "아이동반"] for tag in family_top_result["tags"])
        )
        ...
        family_scores = [place["personalization_score"] for place in family_user_results]
    ...
    async def test_search_preference_adaptation(self) -> None:
        ...
        # 초기 취향: 카페 선호
        initial_behavior = [
            {"action": "search", "query": "카페", "timestamp": "2025-01-01", "satisfaction": 4.0},
            {"action": "visit", "place_category": "cafe", "timestamp": "2025-01-02", "rating": 4.5},
            {"action": "save", "place_category": "cafe", "timestamp": "2025-01-03"},
        ]
        
        # 변화된 취향: 문화 활동 선호로 변화
        recent_behavior = [
            {"action": "search", "query": "전시회", "timestamp": "2025-01-08", "satisfaction": 4.8},
            {"action": "visit", "place_category": "cultural", "timestamp": "2025-01-09", "rating": 5.0},
            {"action": "search", "query": "박물관", "timestamp": "2025-01-10", "satisfaction": 4.6},
            {"action": "save", "place_category": "cultural", "timestamp": "2025-01-10"},
        ]
        
        # Mock 적응형 학습 서비스
        with patch("app.services.adaptive_personalization.update_preferences") as mock_adaptive:
            ...
            def calculate_time_weighted_preferences(all_behavior):
                ...
                for behavior in all_behavior:
                    # 최근 행동에 더 높은 가중치
                    days_ago = (datetime.now() - datetime.fromisoformat(behavior["timestamp"])).days
                    weight = max(0.1, 1.0 - (days_ago * 0.1))  # 시간에 따른 가중치 감소
                    
                    if "place_category" in behavior:
                        category = behavior["place_category"]
                        satisfaction = behavior.get("rating", behavior.get("satisfaction", 3.0))
            ...
            # When: 적응형 개인화 서비스 실행
            from app.services.adaptive_personalization import AdaptivePersonalizationService
        ...
        cultural_change = adapted_result.get("cultural", 0) - initial_result.get("cultural", 0)
    ...
    async def test_personalization_diversity_balance(self) -> None:
        ...
        # Mock 다양성 보장 서비스
        with patch("app.services.diversity_service.apply_diversity_boost") as mock_diversity:
            def apply_diversity_algorithm(results, user_prefs, diversity_factor=0.3):
                ...
                for result in results:
                    ...
                    personal_score = (
                        result["base_score"] * 
                        user_prefs["strong_preferences"].get(result["category"], 0.1)
                    )
                    ...
                    # 최종 점수 계산
                    final_score = personal_score * (1 - diversity_factor) + diversity_boost
                ...
                return sorted(boosted_results, key=lambda x: x["final_score"], reverse=True)
    ...
    async def test_cold_start_personalization(self) -> None:
        ...
        # Mock 콜드 스타트 추천 서비스
        with patch("app.services.cold_start_service.generate_initial_recommendations") as mock_coldstart:
            def cold_start_algorithm(user_profile, available_places):
                """콜드 스타트 알고리즘"""
                
                # 1. 인구통계 기반 선호도
                demographic_prefs = {
                    "20s": {"cafe": 0.4, "activity": 0.3, "cultural": 0.2, "restaurant": 0.1},
                    "30s": {"restaurant": 0.4, "cultural": 0.3, "cafe": 0.2, "activity": 0.1},
                }
                age_prefs = demographic_prefs.get(user_profile["basic_profile"]["age_group"], {})
                
                # 2. 온보딩 선호도 반영
                onboarding_categories = user_profile["onboarding_preferences"]["interested_categories"]
                ...
                for place in available_places:
                    ...
                    # 최종 점수 (가중 평균)
                    final_score = (
                        demo_score * 0.4 +          # 40% 인구통계
                        onboarding_score * 0.3 +    # 30% 온보딩 선호
                        popularity_score * 0.3      # 30% 지역 인기도
                    )
                ...
                return sorted(recommendations, key=lambda x: x["cold_start_score"], reverse=True)
            
            mock_coldstart.return_value = cold_start_algorithm(new_user, self.diverse_places)
        ...
        onboarding_interests = new_user["onboarding_preferences"]["interested_categories"]
    ...
    async def test_personalization_performance_metrics(self) -> None:
        ...
        # Mock 성능 측정 서비스
        with patch("app.services.personalization_metrics.evaluate_recommendation_quality") as mock_metrics:
            def calculate_personalization_metrics(scenarios, recommendations_list):
                ...
                for i, scenario in enumerate(scenarios):
                    ...
                    relevant_count = sum(
                        1 for rec in top_5 
                        if true_prefs.get(rec["category"], 0) > 0.5
                    )
                    ...
                    top_10_categories = {rec["category"] for rec in recommendations[:10]}
                    ...
                    satisfaction = satisfaction / sum(1.0/(i+1) for i in range(10))  # 정규화
                    metrics["user_satisfaction_estimates"].append(satisfaction)
                
                # 평균 계산
                return {
                    "avg_precision_at_5": statistics.mean(metrics["precision_at_k"]),
                    "avg_category_accuracy": statistics.mean(metrics["category_prediction_accuracy"]),
                    "avg_diversity_score": statistics.mean(metrics["diversity_scores"]),
                    "avg_satisfaction_estimate": statistics.mean(metrics["user_satisfaction_estimates"]),
