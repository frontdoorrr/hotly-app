...
class TestCDNIntegration:
    ...
    def test_static_resource_upload(self):
        ...
        def upload_to_cdn(local_path: str, cdn_path: str, content_type: str = None) -> Dict[str, Any]:
            """CDN에 파일 업로드 시뮬레이션"""
        ...
        assert js_result["content_type"] in ["application/javascript", "text/javascript"]
    ...
    def test_cdn_cache_invalidation(self):
        ...
        def check_invalidation_status(invalidation_id: str) -> Dict[str, Any]:
            """무효화 상태 확인"""
            request = next(
                (r for r in invalidation_requests if r["invalidation_id"] == invalidation_id),
                None
            )
...
class TestStaticResourceOptimization:
    ...
    def test_css_minification(self):
        ...
        # 기본적인 CSS 구조는 유지
        assert "body{" in result["minified_content"] or "body {" in result["minified_content"]
    ...
    def test_javascript_bundling(self):
        ...
        bundle_result = bundle_javascript(
            entry_files=entry_files,
            output_path="dist/bundle.min.js",
            minify=True
        )
