...
class TestStructuredLogging:
    ...
    def test_log_correlation_tracing(self):
        ...
        add_trace_log(trace_id, "INFO", "Request started", {"endpoint": "/api/v1/places"})
        add_trace_log(trace_id, "DEBUG", "Database query executed", {"query_time_ms": 45})
...
class TestCentralizedLogManagement:
    ...
    def test_log_aggregation(self):
        ...
        def collect_log(service_name: str, log_entry: Dict[str, Any]):
            ...
            log_collector["log_buffer"].append({
        ...
        # Cache 서비스 로그
        collect_log("hotly-cache", {
            "level": "INFO",
            "message": "Cache hit",
            "cache_key": "places:user-456"
        })
    ...
    def test_log_filtering_and_search(self):
        ...
        # When: 다양한 로그 데이터 추가
        add_log_entry("INFO", "User logged in successfully", "hotly-api", {"user_id": "user-123"})
        add_log_entry("ERROR", "Database connection failed", "hotly-api", {"error": "timeout"})
        add_log_entry("INFO", "Cache hit for user data", "hotly-cache", {"cache_key": "user:123"})
        add_log_entry("WARNING", "AI response confidence low", "hotly-ai", {"confidence": 0.6})
        add_log_entry("ERROR", "AI service unavailable", "hotly-ai", {"retry_count": 3})
        add_log_entry("DEBUG", "Database query executed", "hotly-api", {"query_time_ms": 45})
