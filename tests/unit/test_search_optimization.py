...
class TestSearchOptimization:
    ...
    async def optimization_service(self):
        ...
        service = SearchOptimizationService(
            db=mock_db, cache=cache_service, search_service=mock_search_service
        )
        return service
    ...
    async def performance_service(self):
        ...
        service = SearchPerformanceService(db=mock_db, cache=cache_service)
        return service
    ...
    async def test_search_response_optimization(self, optimization_service):
        ...
        optimized_results = await optimization_service.optimize_search_response(
            results=search_results, config=optimization_config
        )
        ...
        for result in optimized_results:
            ...
            if "description" in result:
                assert (
                    ...
                    <= optimization_config.max_description_length
                )

            # 썸네일 최적화 확인
            if (
                "thumbnail" in result
                and optimization_config.enable_thumbnail_optimization
            ):
    ...
    async def test_pagination_optimization(self, optimization_service):
        ...
        pagination_request = PaginationRequest(
            page=1, page_size=20, enable_cursor_pagination=True, preload_next_page=True
        )
        ...
        paginated_response = await optimization_service.optimize_pagination(
            results=mock_search_results, request=pagination_request
        )
    ...
    async def test_infinite_scroll_support(self, optimization_service):
        ...
        first_page = await optimization_service.get_infinite_scroll_page(
            query="카페", cursor=cursor, page_size=page_size, user_id=uuid4()
        )
...
class TestSearchPerformanceMonitoring:
    ...
    async def test_performance_metrics_collection(self, performance_service):
        ...
        session_id = await performance_service.start_search_session(
            user_id=user_id, query=query, timestamp=start_time
        )
    ...
    async def test_search_performance_analysis(self, performance_service):
        ...
        analysis = await performance_service.analyze_search_performance(
            user_id=user_id, time_period=timedelta(hours=1)
        )
...
class TestSearchAutocompleteOptimization:
    ...
    async def test_autocomplete_caching(self, optimization_service):
        ...
        if suggestions is None:
            # 자동완성 생성 및 캐싱
            fresh_suggestions = ["스타벅스", "스타벅스 강남점", "스타트업 카페", "스타일러 맛집", "스탠다드 바"]
        ...
        if suggestions:
            ...
            assert any(
                partial_query in suggestion.lower() for suggestion in suggestions
            )
    ...
    async def test_personalized_autocomplete(self, optimization_service):
        ...
        # Given: 사용자별 검색 히스토리
        user_id = uuid4()
        search_history = ["스타벅스 강남점", "스타벅스 역삼점", "투썸플레이스", "카페 베네"]
        ...
        personalized_suggestions = (
            await optimization_service.get_personalized_autocomplete(
                partial_query="스타", user_id=user_id, max_suggestions=3
            )
        )
...
if __name__ == "__main__":
    ...
    async def run_tests():
        ...
        optimization_service = SearchOptimizationService(
            db=AsyncMock(), cache=cache_service, search_service=AsyncMock()
        )

        performance_service = SearchPerformanceService(
            db=AsyncMock(), cache=cache_service
        )
        ...
        try:
            ...
            await test_optimization.test_search_response_optimization(
                optimization_service
            )
            await test_optimization.test_pagination_optimization(optimization_service)

            await test_performance.test_performance_metrics_collection(
                performance_service
            )
