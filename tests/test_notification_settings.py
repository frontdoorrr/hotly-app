...
from app.models.notification import NotificationType, UserNotificationSettings
...
class TestNotificationSettingsService:
    ...
    def sample_settings_create(self):
        ...
        return UserNotificationSettingsCreate(
            ...
            quiet_hours=QuietHours(
                enabled=True, start=time(22, 0), end=time(8, 0), weekdays_only=False
            ),
            ...
            personalization=PersonalizationSettings(
                enabled=True, frequency_limit_per_day=8, frequency_limit_per_week=40
            ),
        )
    ...
    async def test_create_notification_settings_success(
        self, settings_service, sample_settings_create
    ):
        """
        ...
        settings_service.db.query.return_value.filter.return_value.first.return_value = (
            None
        )
        ...
        # When
        result = await settings_service.create_user_settings(
            user_id, sample_settings_create
        )
        ...
        assert (
            added_settings.quiet_hours_enabled
            == sample_settings_create.quiet_hours.enabled
        )
        assert (
            added_settings.quiet_hours_start == sample_settings_create.quiet_hours.start
        )
        assert (
            added_settings.day_before_hour
            == sample_settings_create.timing.day_before_hour
        )
        assert (
            added_settings.frequency_limit_per_day
            == sample_settings_create.personalization.frequency_limit_per_day
        )

    async def test_create_duplicate_settings_fails(
        self, settings_service, sample_settings_create, sample_settings_model
    ):
        """
    ...
    async def test_get_user_settings_success(
        self, settings_service, sample_settings_model
    ):
        """
    ...
    async def test_update_user_settings_success(
        ...
        update_data = UserNotificationSettingsUpdate(
            ...
            timing=NotificationTiming(
                day_before_hour=20, departure_minutes_before=60  # 20시로 변경  # 60분 전으로 변경
            ),
        )
        ...
        assert result.quiet_hours_weekdays_only is True
    ...
    async def test_update_user_settings_partial_update(
        ...
        assert result.quiet_hours_enabled == sample_settings_model.quiet_hours_enabled
        assert (
            result.date_reminder_enabled == sample_settings_model.date_reminder_enabled
        )
    ...
    async def test_is_notification_allowed_success(self, sample_settings_model):
        ...
        # 활성화된 알림 타입들
        assert (
            settings.is_notification_allowed(
                NotificationType.DEPARTURE_REMINDER, test_time
            )
            is True
        )
        assert (
            settings.is_notification_allowed(
                NotificationType.PREPARATION_REMINDER, test_time
            )
            is True
        )
        assert settings.is_notification_allowed("business_hours", test_time) is True
        assert settings.is_notification_allowed("weather", test_time) is True
        assert settings.is_notification_allowed("recommendations", test_time) is True

        # 비활성화된 알림 타입들
        assert (
            settings.is_notification_allowed(NotificationType.MOVE_REMINDER, test_time)
            is False
        )
        assert settings.is_notification_allowed("traffic", test_time) is False
        assert (
            settings.is_notification_allowed(NotificationType.PROMOTIONAL, test_time)
            is False
        )
    ...
    async def test_is_notification_allowed_quiet_time(self, sample_settings_model):
        ...
        assert (
            settings.is_notification_allowed(
                NotificationType.DEPARTURE_REMINDER, quiet_time
            )
            is False
        )
        assert (
            settings.is_notification_allowed(
                NotificationType.PREPARATION_REMINDER, quiet_time
            )
            is False
        )
        assert settings.is_notification_allowed("weather", quiet_time) is False

    async def test_is_notification_allowed_disabled_notifications(
        self, sample_settings_model
    ):
        """
        ...
        assert settings.is_notification_allowed("weather", test_time) is False
        assert settings.is_notification_allowed("recommendations", test_time) is False

    async def test_is_notification_allowed_weekday_only_quiet_hours(
        self, sample_settings_model
    ):
        """
        ...
        assert (
            settings.is_notification_allowed(
                NotificationType.DEPARTURE_REMINDER, weekend_quiet_time
            )
            is True
        )
        assert settings.is_notification_allowed("weather", weekend_quiet_time) is True
...
class TestNotificationSettingsIntegration:
    ...
    async def test_settings_creation_to_notification_filtering_flow(self):
        ...
        settings_create = UserNotificationSettingsCreate(
            ...
            quiet_hours=QuietHours(enabled=True, start=time(22, 0), end=time(8, 0)),
        )
        ...
        # 평상시: 활성화된 알림만 허용
        assert (
            settings_model.is_notification_allowed(
                NotificationType.PREPARATION_REMINDER, normal_time
            )
            is True
        )
        assert (
            settings_model.is_notification_allowed(
                NotificationType.DEPARTURE_REMINDER, normal_time
            )
            is False
        )
        assert settings_model.is_notification_allowed("weather", normal_time) is True
        assert settings_model.is_notification_allowed("traffic", normal_time) is False
        assert (
            settings_model.is_notification_allowed(
                NotificationType.PROMOTIONAL, normal_time
            )
            is False
        )
    ...
    async def test_personalization_settings_affect_frequency_limits(self):
        ...
        assert settings.personalized_timing_enabled is True
