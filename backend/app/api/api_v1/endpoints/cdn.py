...
from app.services.content.cdn_service import (
    get_cache_manager,
    get_cdn_manager,
...
@router.get(
    "/cache-headers", summary="캐시 헤더 생성", description="파일 타입에 맞는 적절한 캐시 헤더를 생성합니다."
)
async def generate_cache_headers(
    ...
    try:
        ...
        headers = cache_manager.generate_cache_headers(
            file_path=file_path, cache_duration=cache_duration
        )
...
async def generate_preload_hints(request: PreloadHintsRequest):
    ...
    try:
        ...
        preload_tags = cache_manager.generate_preload_hints(request.critical_resources)
...
@router.get(
    "/health", summary="CDN 서비스 상태", description="CDN 및 리소스 최적화 서비스의 상태를 확인합니다."
)
