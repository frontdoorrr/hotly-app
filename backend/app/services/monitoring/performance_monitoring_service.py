...
class PerformanceDashboard:
    ...
    def update_dashboard_metrics(self, metrics: Dict[str, Any]):
        ...
        with self._lock:
            ...
            self.metrics_history.append({**metrics, "timestamp": timestamp})
    ...
    def analyze_performance_trends(self, hours: int = 1) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_response_time = sum(
                m.get("response_time_avg", 0) for m in recent_metrics
            ) / len(recent_metrics)
            avg_cache_hit_rate = sum(
                m.get("cache_hit_rate", 0) for m in recent_metrics
            ) / len(recent_metrics)
            avg_error_rate = sum(m.get("error_rate", 0) for m in recent_metrics) / len(
                recent_metrics
            )
            ...
            if len(recent_metrics) >= 2:
                ...
                response_time_trend = (
                    "improving"
                    if last.get("response_time_avg", 0)
                    < first.get("response_time_avg", 0)
                    else (
                        "degrading"
                        if last.get("response_time_avg", 0)
                        > first.get("response_time_avg", 0)
                ...
                cache_trend = (
                    "improving"
                    if last.get("cache_hit_rate", 0) > first.get("cache_hit_rate", 0)
                    else (
                        "degrading"
                        if last.get("cache_hit_rate", 0)
                        < first.get("cache_hit_rate", 0)
                        else "stable"
                    )
                )
                error_trend = (
                    "improving"
                    if last.get("error_rate", 0) < first.get("error_rate", 0)
                    else (
                        "degrading"
                        if last.get("error_rate", 0) > first.get("error_rate", 0)
                        else "stable"
...
class PerformanceOptimizer:
    ...
    def detect_slow_query(
        self, query: str, execution_time_ms: float, params: Dict = None
    ):
        """느린 쿼리 감지"""
    ...
    def collect_resource_metrics(
        self, cpu: float, memory: float, request_rate: int, response_time: float
    ):
        """리소스 메트릭 수집"""
        with self._lock:
    ...
    def generate_scaling_recommendations(self) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            avg_cpu = sum(self.resource_metrics["cpu_usage"]) / len(
                self.resource_metrics["cpu_usage"]
            )
            avg_memory = sum(self.resource_metrics["memory_usage"]) / len(
                self.resource_metrics["memory_usage"]
            )
            avg_request_rate = sum(self.resource_metrics["request_rate"]) / len(
                self.resource_metrics["request_rate"]
            )
            avg_response_time = sum(self.resource_metrics["response_time"]) / len(
                self.resource_metrics["response_time"]
            )
...
class APMIntegrationService:
    ...
    def monitor_query_performance(
        ...
        """쿼리 성능 모니터링"""
        self.metrics_collector.monitor_query_performance(
    ...
    def track_endpoint_performance(
        ...
        """엔드포인트 성능 추적"""
        self.optimizer.track_endpoint_performance(
