...
from app.schemas.search_optimization import SearchPerformanceMetrics, SearchSessionInfo
...
class SearchPerformanceService:
    ...
    async def start_search_session(
        ...
        try:
            ...
            await self.cache.set(session_key, session_info.dict(), ttl=3600)  # 1시간
    ...
    async def record_search_metrics(self, metrics: SearchPerformanceMetrics) -> bool:
        ...
        try:
            ...
            if alert_triggered:
                logger.warning(
                    f"Performance alert triggered for session: {metrics.session_id}"
                )
    ...
    async def get_search_metrics(
        self, session_id: str
    ) -> Optional[SearchPerformanceMetrics]:
        """
        검색 메트릭 조회
    ...
    async def analyze_search_performance(
        self, user_id: UUID, time_period: timedelta
    ) -> Dict[str, Any]:
        """
        검색 성능 분석
        ...
        try:
            ...
            logger.info(
                f"Analyzed performance for user {user_id}: {analysis['total_searches']} searches"
            )
            return analysis
    ...
    async def check_performance_alert(self, metrics: SearchPerformanceMetrics) -> bool:
        ...
        try:
            ...
            if (
                ...
                and metrics.database_query_time_ms
                > self.performance_thresholds["max_database_query_time_ms"]
            ):
                alerts.append(
                    {
                        "metric": "database_query_time",
                        "threshold": self.performance_thresholds[
                            "max_database_query_time_ms"
                        ],
            ...
            if (
                metrics.elasticsearch_query_time_ms
                and metrics.elasticsearch_query_time_ms
                > self.performance_thresholds["max_elasticsearch_query_time_ms"]
            ):
                alerts.append(
                    {
                        "metric": "elasticsearch_query_time",
                        "threshold": self.performance_thresholds[
                            "max_elasticsearch_query_time_ms"
                        ],
    ...
    async def generate_ui_optimization_report(
        ...
        try:
            ...
            # 기본 성능 통계
            performance_stats = await self._calculate_detailed_performance_stats(
                user_metrics
            )

            # UI 상호작용 메트릭 계산
            ui_metrics = await self._calculate_ui_interaction_metrics(
                user_id, time_period
            )
            ...
            report = {
                ...
                "infinite_scroll_usage_rate": ui_metrics.get(
    ...
    async def track_search_ab_test_metrics(
        self, user_id: UUID, variant: str, metrics: Dict[str, Any]
    ) -> bool:
        """
        A/B 테스트 메트릭 추적
        ...
        try:
            ...
            logger.debug(
                f"Tracked A/B test metrics for user {user_id}, variant {variant}"
            )
            return True
    ...
    async def get_search_trend_analysis(self, time_period: timedelta) -> Dict[str, Any]:
        """
        검색 트렌드 분석
        ...
        try:
            ...
            for query, count in sorted(
                ...
                trend = (
                    "rising" if count > 50 else "stable" if count > 20 else "falling"
                )
            ...
            trend_analysis = {
                ...
                "declining_queries": [
                    q for q in trending_queries if q["trend"] == "falling"
                ][:5],
    ...
    async def calculate_search_quality_score(
        ...
        try:
            ...
            if results:
                ...
                recent_results = sum(
                    1 for r in results if r.get("updated_recently", False)
                )
            ...
            quality_score = {
                ...
                "factors": {
                    ...
                    "user_feedback_rating": user_feedback.get("rating")
                    if user_feedback
                    else None,
    ...
    async def _record_alert(
        self, metrics: SearchPerformanceMetrics, alert_data: Dict[str, Any]
    ) -> None:
        """알림 기록"""
        try:
            alert = {
