...
class FirebaseAuthService:
    ...
    def _initialize_firebase(self):
        ...
        try:
            ...
            if not settings.FIREBASE_CREDENTIALS_PATH:
                logger.warning(
                    "Firebase credentials path not set, using mock for development"
                )
                return None
    ...
    async def _authenticate_social_user(
        self, request: SocialLoginRequest
    ) -> Optional[Dict]:
        """소셜 제공자별 사용자 인증"""
        try:
    ...
    async def _create_user_profile(
        self, user_info: Dict, provider: SocialProvider
    ) -> UserProfile:
        """사용자 프로필 생성"""
        # 게스트 사용자 권한 설정
    ...
    async def create_user_session(
        self, user_id: str, device_id: str, session_token: str, expires_at: datetime
    ) -> bool:
        """사용자 세션 생성"""
        try:
            session_info = {
    ...
    async def create_anonymous_user(self, device_id: str) -> LoginResponse:
        ...
        try:
            ...
            user_profile = await self._create_user_profile(
                user_info, SocialProvider.ANONYMOUS
            )
    ...
    async def upgrade_anonymous_user(
        self, anonymous_user_id: str, social_login: SocialLoginRequest
    ) -> LoginResponse:
        ...
        try:
            ...
            if self.firebase_auth.link_provider:
                linked_user = self.firebase_auth.link_provider(
                    anonymous_user_id, user_info
                )
                user_info.update(
                    {
                        "uid": linked_user["uid"],
            ...
            # 업그레이드된 사용자 프로필 생성
            user_profile = await self._create_user_profile(
                user_info, social_login.provider
            )
            user_profile.is_anonymous = False
            user_profile.linked_providers = [
                SocialProvider.ANONYMOUS,
