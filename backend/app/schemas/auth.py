...
class AuthError(str, Enum):
    ...
    INVALID_TOKEN = "invalid_token"  # nosec - 에러 코드 상수, 비밀번호 아님
    TOKEN_EXPIRED = "token_expired"  # nosec - 에러 코드 상수, 비밀번호 아님
...
class UserProfile(BaseModel):
    ...
    linked_providers: List[SocialProvider] = Field(
        default_factory=list, description="연결된 제공자 목록"
    )
    ...
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="계정 생성 시간"
    )
    last_login_at: Optional[datetime] = Field(None, description="마지막 로그인 시간")
    permissions: UserPermissions = Field(
        default_factory=UserPermissions, description="사용자 권한"
    )
...
class SignUpRequest(BaseModel):
    ...
    display_name: str = Field(..., description="사용자 이름")
...
class UserSession(BaseModel):
    ...
    last_activity_at: datetime = Field(
        default_factory=datetime.utcnow, description="마지막 활동 시간"
    )
