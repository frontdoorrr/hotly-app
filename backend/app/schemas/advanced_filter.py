...
class LocationFilter(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"lat": 37.5563, "lng": 126.9225, "radius_km": 2.0}}
...
class AdvancedFilterRequest(BaseModel):
    ...
    def validate_rating_range(cls, v, values):
        if (
            v is not None
            and "rating_min" in values
            and values["rating_min"] is not None
        ):
...
class FilterFacets(BaseModel):
    ...
    visit_status: List[FacetOption] = Field(
        default_factory=list, description="방문 상태 패싯"
    )
    rating_ranges: List[FacetOption] = Field(
        default_factory=list, description="평점 구간 패싯"
    )
...
class FilterSuggestions(BaseModel):
    ...
    alternative_filters: List[Dict[str, Any]] = Field(
        default_factory=list, description="대안 필터 조건"
    )
    popular_filters: List[Dict[str, Any]] = Field(
        default_factory=list, description="인기 필터 조합"
    )
...
class PerformanceMetrics(BaseModel):
    ...
    elasticsearch_took: Optional[int] = Field(
        None, description="Elasticsearch 실행 시간 (ms)"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "facets": {
                    "categories": [{"name": "cafe", "count": 25, "selected": True}]
                },
                "applied_filters": {"categories": ["cafe"], "regions": ["마포구"]},
                "query_info": {
                    "total_hits": 127,
                    "max_score": 3.2,
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "performance_stats": {"avg_response_time": 145, "cache_hit_rate": 0.67},
            }
        }
