...
# Missing Request Schemas for preference_setup.py


class BudgetPreferenceRequest(BaseModel):
...
class CategorySelectionRequest(BaseModel):
    """Request for initial category selection."""

    user_id: str
    selected_categories: List[str] = Field(..., min_items=2, max_items=5)
    selection_reason: str
    confidence_level: float = Field(..., ge=0.0, le=1.0)


class LocationPreferenceRequest(BaseModel):
    """Request for location preference setup."""

    user_id: str
    preferred_areas: List[Dict[str, Any]]
    travel_range_km: int = Field(..., ge=1, le=50)
    transportation_preferences: List[str]


class PreferenceValidationRequest(BaseModel):
    """Request for preference validation."""

    user_id: str
    preferences: Dict[str, Any]


class PreferenceScoringRequest(BaseModel):
    """Request for preference scoring."""
...
class CategorySelectionResponse(BaseModel):
    """Response for category selection."""

    user_id: str
    selected_categories: List[str]
    categories_configured: bool = True
...
class PreferenceScoringResponse(BaseModel):
    """Response for preference scoring."""

    user_id: str
    calculated_scores: Dict[str, float]
    weighted_categories: Dict[str, Dict[str, float]]
    total_score: float
...
# Sample Guide Service Schemas


class SamplePlaceSchema(BaseModel):
    """Schema for sample places."""
...
class FirstCourseGuideResponse(BaseModel):
    """Response for first course guide."""
