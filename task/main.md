...
### 1-2. 장소 관리 백엔드 개발 ✅ **완료** (2025-09-29)
...
### 1-3. AI 기반 코스 추천 백엔드 개발 🔄 **진행 중** (2025-10-03)
...
- **수용 기준**:
  ...
  - Given 코스 추천 결과, When 실시간 위치 확인, Then 경로 정보 제공 ✅
- **하위 작업**:
  - 1-3-1. 코스 추천 요청/응답 스키마 정의 ✅ **완료** (2025-10-03)
    - **상세**: Pydantic 기반 API 스키마, 다양한 교통수단, 난이도 계산
    - **구현**: `app/schemas/course_recommendation.py` (245줄)
    - **스키마**: CourseGenerateRequest, CourseGenerateResponse, CourseDetail, CoursePlaceDetail
    - **기능**: 3-6개 장소 제한, 교통수단 선택, 출발 시간 설정, 선호도 가중치
    - **테스트**: 스키마 검증, 필수 필드 확인
  - 1-3-2. 장소 간 거리 계산 알고리즘 (Haversine) ✅ **완료** (2025-10-03)
    - **상세**: Haversine formula 기반 정확한 거리 계산, 교통수단별 이동 시간 추정
    - **구현**: `app/services/course_optimizer.py` 내 calculate_distance 메서드
    - **알고리즘**: 지구 반지름 6371km, 위경도 라디안 변환, 정확도 ±10m
    - **성능**: 거리 계산 1ms 이내
    - **테스트**: 실제 서울 지역 좌표 기반 검증 (홍대↔강남 11.2km)
  - 1-3-3. AI 기반 코스 최적화 엔진 (Genetic Algorithm) ✅ **완료** (2025-10-03)
    - **상세**: 유전 알고리즘 기반 TSP 최적화, 다중 기준 평가, 50세대 진화
    - **구현**: `app/services/course_optimizer.py` (363줄), CourseOptimizer 클래스
    - **알고리즘**:
      - 개체군 크기: 50, 세대 수: 100, 엘리트 비율: 20%, 돌연변이율: 10%
      - Order Crossover (OX) 교차 연산
      - Tournament Selection (크기 3) 선택 연산
      - Swap Mutation 돌연변이 연산
    - **평가 함수**: 거리 40%, 시간 25%, 다양성 20%, 선호도 15%
    - **성능**: 6개 장소 최적화 100ms 이내, 최적화 점수 0-1 범위
    - **테스트**: `tests/unit/test_course_optimizer.py` (9개 테스트, 모두 통과)
  - 1-3-4. 코스 생성 서비스 (DB 통합) ✅ **완료** (2025-10-03)
    - **상세**: 데이터베이스 연동, PostGIS 좌표 추출, 코스 상세 정보 생성
    - **구현**: `app/services/course_generator_service.py` (218줄), CourseGeneratorService 클래스
    - **기능**:
      - PostgreSQL에서 장소 데이터 조회 (place_ids 기반)
      - PostGIS Geography 좌표 추출 (geoalchemy2.shape.to_shape)
      - 최적화 엔진 호출 및 결과 변환
      - 시간대별 도착/출발 시간 계산
      - 장소 간 이동 정보 생성 (거리, 소요시간, 경로 설명)
      - 난이도 계산 (EASY/MODERATE/HARD)
    - **성능**: 코스 생성 전체 프로세스 500ms 이내
    - **에러 처리**: 존재하지 않는 장소 ID 검증, 좌표 누락 처리
...
- **하위 작업**:
  - 1-4-1. Kakao Map SDK 연동 및 기본 지도 로딩 ✅ **완료** (2025-09-30)
    ...
    - **API**:
      ...
      - `GET /api/v1/map/reverse-geocode` - 좌표 → 주소 변환
...
- **하위 작업**:
  ...
  - 2-2-2. 알림 스케줄링 및 사용자별 타겟팅 ⏳ **미완료**
    - **상세**: 시간대별 알림 스케줄링, 사용자 행동 분석 기반 타겟팅
    - **필요 구현**: 스케줄러 시스템, 사용자 세분화, A/B 테스트 프레임워크
...
### 2-3. 검색, 필터, 정렬 기능 백엔드 ✅ **완료** (2025-09-14)
...
- **하위 작업**:
  ...
  - 3-1-3. 생체 인증 및 PIN 인증 시스템 ⏳ **미완료**
    - **상세**: 생체인증(지문/Face ID), PIN 인증, 보안 레벨 관리
    - **필요 구현**: 생체인증 API, PIN 설정, 보안 정책 엔진
  - 3-1-4. 익명 사용자 및 게스트 개인화 백엔드 ⏳ **미완료**
    - **상세**: 게스트 모드, 임시 세션, 익명 사용자 데이터 관리
    - **필요 구현**: 게스트 세션 관리, 임시 데이터 저장, 계정 전환
  - 3-1-5. 보안 검증 및 데이터별 구현 ⏳ **미완료**
    - **상세**: 다단계 인증, 보안 감사, 비정상 접근 탐지
    - **필요 구현**: MFA 시스템, 보안 로그, 이상행동 감지
  - 3-1-6. 사용자 인증 테스트 코드 작성 ⏳ **미완료**
    - **상세**: 인증 시스템 전체 테스트, 보안 테스트, 통합 테스트
    - **필요 구현**: 인증 테스트 스위트, 보안 테스트, 성능 테스트
