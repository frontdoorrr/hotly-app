...
2-13. 문서화
 ...
 5-4. 데이터/직렬화
     ...
     - `ObjectId`는 문자열로 노출(`id`), 인덱스/TTL 전략은 스키마에 명시.

 6. 데이터(MongoDB)
 6-1. 네이밍/스키마
     - 컬렉션 복수형(`places`, `courses`). 필드 `snake_case`(내부).
     - 인덱스/고유키/TTL은 마이그레이션/스키마 정의에 선언적으로 기술.
 6-2. 중복/정규화
     - 장소 중복 키: 이름+주소 정규화+지오 해시 라운딩. 퍼지 매칭 시 임계치 고정.
 6-3. 트랜잭션/일관성
     - 멱등 작성 연산(업서트) 우선. 다중 문서 일관성 필요 시 세션/트랜잭션.
